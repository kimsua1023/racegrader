...
## 새 의존성(라이브러리) 추가 시 — 2단계 라이선스 체크

**1단계 — 라이브러리 도입 전 (육안 확인)**

새 Go 패키지를 쓰기로 결정하기 *전에*, pkg.go.dev 또는 GitHub 저장소 페이지에서
라이선스를 먼저 확인합니다. **permissive 계열(MIT, BSD, Apache-2.0 등)만 통과**시키고,
GPL/AGPL 등 viral 라이선스는 이 단계에서 바로 걸러냅니다.

**2단계 — PR 올리기 전 (자동 검증)**

1단계를 통과한 패키지를 실제로 코드에 추가했다면, 머지 전에 아래를 실행하고
결과를 PR에 첨부하세요.
...
GPL/AGPL 계열이 걸리면 **팀원3(정수민)에게 즉시 공유** 후 대체 라이브러리를 검토합니다.
커널(`kernel/`) 쪽에 코드를 가져다 붙일 때도 1단계·2단계 모두 동일하게 적용해,
GPL 코드가 섞이는지 반드시 확인합니다.
